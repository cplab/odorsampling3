...
class GlomLayer(list[cells.Glom]):
    ...
    # Resets the activation levels and recurrent connection maps of all Glom cells in the layer.
        ...
        """def glom_generator():
            with open(name) as f:
                for line in f:
                    ...
                    if data:
                        yield cells.Glom(int(data.group(1)), float(data.group(2)),
                                        (int(data.group(3)), int(data.group(4))),
                                        int(data.group(5)))

            glom_layer = list(glom_generator())
...
# NOTE: Make sure that this was replaced properly
    ...
    def _buildWeights(self, bias: str, scale: int) -> list[int]:
        ...
        weights: list[int] = []
        counter = 0
        # If the bias is "lin", the scale is added as is to the weights
        if bias == "lin":
            while counter < len(self):
                weights.append(scale)
                counter += 1
        # For any other bias value, 2 raised to the power of scale is added to the weights.
        else:
            while counter < len(self):
...
class MitralLayer(list[cells.Mitral]):
    ...
    for m in self:
        ...
        loc = str(x) + ":" + str(y)
        # Storing glom
        glom = '+'.join(f"{k}|{v}" for k, v in m.glom.items())
        st = f"{st}{m.id},{m.activ},{loc},{glom};\n"
    ...
    def save(self, name: str):
        ...
        for m in self:
            ...
            while i < end - 1:
                # Find the indices of the opening and closing parentheses and the comma
                ind1 = glom.index("(", i)
                comma = glom.index(",",ind1)
                ind2 = glom.index(")", ind1)
                # Extract the string between the parentheses and replace the comma with a pipe
                s = s + glom[ind1 + 1:comma] + "|" + glom[comma + 2:ind2] + "+"
                # Move the index to the end of the current item
                i = ind2
             # Add the mitral cell ID, activation, location, and glomerulus items to the string
            st = f"{st}{m.id},{m.activ},{loc},{s};\n"
        # Create the filename with the .MCL extension    
        filename = f"{name}{config.ML_EXT}"
        with open(filename, 'w') as f:
            # Log that the mitral layer has been saved
            logger.info("Mitral layer saved to `%s`.", filename)
    ...
    def load(cls, name: str) -> MitralLayer:
        ...
        precondition: name is a string with correct extension"""
        assert type(name) == str, "name isn't a string"
        # Compile a regular expression pattern to match the data in the file
        pattern = re.compile(r"(\d+),(\d+),(\d+):(\d+),(\d+);")
        ...
        with open(name, 'r') as f:
            for line in f.readlines():
                ...
                if not data:
                    continue
                # If the pattern matches, create a new Mitral cell with the data from the line
                mcl.append(
...
# TODO: rewrite samplers
        ...
        ***Weights of a mitral cell's gloms add up to 1.0***
        ...
        for mitral in self:
            ...
            if int(num_layers) == 1:
                ...
                # Loop over the x and y coordinates within the bounds
                while x_inner <= x_outer:
                    y_inner = y - 1
                    while y_inner <= y_outer:
                        # If the coordinates are not the center, add them to the first layer
                ...
                for selected in range(num_to_select):
                    # Recalc act
                    # If this is the last glomerulus to select, set the weight to the leftover weight
                    if selected == num_to_select - 1:
                        weight = leftover
                    else:
                        # Otherwise, generate a random weight
                        weight = utils.RNG.uniform(0, leftover)
                        leftover -= weight
    ...
    def biasSample(self, gl: GlomLayer, cr, fix, bias: str, sd = 0) -> ConnMap:
        ...
        # Calculate the scale based on the size of the layers and the connection ratio
        scale = max(MIN_SCALE, math.floor((len(self)/len(gl)) * SCALE_FACTOR * cr))
        #Build weights
        weights = gl._buildWeights(bias, scale)
        # Calculate the sum of the weights based on the bias
        ...
        # start looping through each mitral cell
        for mitral_idx in range(len(self)):
            # Generate new cr per mitral `if not fix``.
            if not fix:
                cr = min(max(int(utils.RNG.normal(cr_orig, sd)), 1), len(gl))
            
            gl_indexes: list[int] = []
            
            # start connecting mitral cell to (multiple) glom
            for _ in range(cr):
                # Select a glomerulus based on the weights
                glom_weight_idx = _select_glom(utils.RNG.integers(1, s, endpoint = True), weights, 0)
                
                # If the glomerulus has already been selected, choose a new one             
                ...
                # Add the mitral cell index, glomerulus index, and a random weight to the map
                map_.append((mitral_idx, glom_weight_idx, utils.RNG.uniform(0, .4)))
                
                # Add the selected glomerulus to the list
                gl_indexes.append(glom_weight_idx)

                # Recalculate the weights and the sum of the weights
    ...
    def graph_activation(self, gl: GlomLayer, n, m):
        ...
        # Loop over each mitral cell in the layer
        for mitral in self:
            # If the location of the mitral cell is already in the dictionary, 
            # increment the count and append the activation
            ...
            # If the location of the mitral cell is not in the dictionary, 
            # add it with a count of 1 and the activation
            else:
        ...
        # Initialize a 2D list (graph) with dimensions m * maxMitrals with all elements as 0
        counter = 0
        ...
        # Loop over the range of m (the number of mitral cells)
        for x in range(m):
            # Loop over the length of the graph
            for y in range(len(graph)):
                # Calculate the key for the mitral activations dictionary
                key = str((x,math.floor(y / maxMitrals)))
                # If the key is in the mitral activations dictionary, 
                # update the corresponding element in the graph
                if key in mitralActivations:
                    numActivations = len(mitralActivations.get(key))
                    
                    # If there is only one activation, set the graph element to that activation
                    if numActivations == 1:
                        graph[y][x] = mitralActivations.get(key)[0]
                    
                    # If the number of activations is less than the maximum number of mitrals,
                    # set the graph element based on the modulus of the y-coordinate and the maximum number of mitrals
                    ...
                    # set the graph element based on the modulus of the y-coordinate and the number of activations
                    else:
    ...
    """def loadMCLSamplingMap(name: str) -> list[tuple[int, int, float]]:
        ...
        with open(name, 'r') as file:
            for line in file.readlines():
                ...
                if match:
                    x, y, z = match.groups()
                    map_.append((int(x), int(y), float(z)))
        return map_    
    """
...
def apply_sample_map(gl: GlomLayer, mcl: MitralLayer, map_: list[list[int]]) -> tuple[MitralLayer, GlomLayer]:
    ...
    # assert isinstance(map_, list) and all(isinstance(sublist, list) and all(isinstance(item, int) 
    # for item in sublist) for sublist in map_), "Map is not a list of lists of integers"
...
"""
    def heap_sort(layer):
        #sorts layer(heap sort) from highest act lvl to lowest.
        #Precondition: Layer is a valid GL or MCL
        # Convert layer into a heap
        heapq.heapify(layer)
        ...
        while layer:
            sorted_layer.append(heapq.heappop(layer))

        # Reverse the sorted list to get a descending order
        sorted_layer.reverse()

        # Replace the original layer with the sorted layer
        layer[:] = sorted_layer
"""
